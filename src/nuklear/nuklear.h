...
typedef void(*nk_my_plugin_copy)(nk_handle, struct TextLine*, int numLines, struct TextCursor start, struct TextCursor end);

struct nk_my_text_edit;
struct nk_my_clipboard {
	nk_handle userdata;
	nk_my_plugin_paste paste;
	nk_my_plugin_copy copy;
};				
...
struct nk_command_colored_text {
	struct nk_command header;
	const struct nk_user_font *font;
	struct nk_color background;
	short x, y;
	unsigned short w, h;
	float height;
	const struct nk_color* colorTable;
	int stringLen;
	int colorLen;
	char colorString[1];//The string data is first, then the color data
};
...
/* colored text */
NK_API void nk_draw_colored_text(struct nk_command_buffer*, struct nk_rect, const char *text, int textLen, const char* color, int colorLen, const struct nk_user_font*, struct nk_color);
...
NK_API void nk_draw_list_add_colored_text(struct nk_draw_list*, const struct nk_user_font*, struct nk_rect, const char *text, int textLen, const char* color, int colorLen, const struct nk_color* colorTable, float font_height);
#ifdef NK_INCLUDE_COMMAND_USERDATA
...
struct nk_style {
	...
	enum nk_style_cursor cursor_type;
...
/* public: can be accessed freely */
    ...
    struct nk_clipboard clip;
	struct nk_my_clipboard my_clip;
...
nk_draw_colored_text(struct nk_command_buffer *b, struct nk_rect r,
	const char *string, int stringLen, const char* color, int colorLen, const struct nk_color* colorTable,
	const struct nk_user_font *font, struct nk_color bg)
{
	float text_width = 0;
	struct nk_command_colored_text *cmd;

	NK_ASSERT(b);
	NK_ASSERT(font);
	if (!b || !string || !stringLen) return;
	if (b->use_clipping) {
		const struct nk_rect *c = &b->clip;
		if (c->w == 0 || c->h == 0 || !NK_INTERSECT(r.x, r.y, r.w, r.h, c->x, c->y, c->w, c->h))
			return;
	...
	/* make sure text fits inside bounds */
	text_width = font->width(font->userdata, font->height, string, stringLen);
	if (text_width > r.w) {
		int glyphs = 0;
		float txt_width = (float)text_width;
		stringLen = nk_text_clamp(font, string, stringLen, r.w, &glyphs, &txt_width, 0, 0);
	}

	if (!stringLen) return;
	cmd = (struct nk_command_colored_text*)
		nk_command_buffer_push(b, NK_COMMAND_COLORED_TEXT, sizeof(*cmd) + (nk_size)(stringLen + 1) + (nk_size)(colorLen));
	if (!cmd) return;
	cmd->x = (short)r.x;
	cmd->y = (short)r.y;
	cmd->w = (unsigned short)r.w;
	...
	NK_MEMCPY(cmd->colorString, string, (nk_size)stringLen);
	cmd->colorString[stringLen] = '\0';
	cmd->colorLen = colorLen;
	NK_MEMCPY(cmd->colorString + stringLen + 1, color, (nk_size)colorLen);
	cmd->colorTable = colorTable;
}
...
nk_draw_list_add_colored_text(struct nk_draw_list *list, const struct nk_user_font *font,
	...
	const char* colors, int colorLen, const struct nk_color* colorTable, float font_height, struct nk_color bg)
{
	float x = 0;
	int text_len = 0;
	nk_rune unicode = 0;
	...
	int next_glyph_len = 0;
	struct nk_user_font_glyph g;
	nk_color currentColor;
	int colorIndex = 0;

	NK_ASSERT(list);
	if (!list || !textLen || !text) return;
	if (!NK_INTERSECT(rect.x, rect.y, rect.w, rect.h,
		list->clip_rect.x, list->clip_rect.y, list->clip_rect.w, list->clip_rect.h)) {
		return;
	}
	struct nk_rect bgRect = rect;
	bgRect.h *= 2;//TODO: investigate why this is needed
	nk_draw_list_fill_rect(list, bgRect, bg, 0);
	nk_draw_list_push_image(list, font->texture);
	x = rect.x;
	glyph_len = nk_utf_decode(text, &unicode, textLen);
	if (!glyph_len) return;

	/* draw every glyph image */
	//fg.a = (nk_byte)((float)fg.a * list->config.global_alpha);

	while (text_len < textLen && glyph_len) {
		float gx, gy, gh, gw;
		float char_width = 0;
		if (unicode == NK_UTF_INVALID) break;

		/* query currently drawn glyph information */
		next_glyph_len = nk_utf_decode(text + text_len + glyph_len, &next, (int)textLen - text_len);
		if (unicode == '\t') {
		...
		NK_ASSERT(colorIndex < colorLen);
		currentColor = colorTable[colors[colorIndex]];
		nk_draw_list_push_rect_uv(list, nk_vec2(gx, gy), nk_vec2(gx + gw, gy + gh),
			g.uv[0], g.uv[1], currentColor);
    ...
    const struct nk_convert_config *config)
{
    ...
    nk_foreach(cmd, ctx)
    {
		...
		case NK_COMMAND_COLORED_TEXT: {
			const struct nk_command_colored_text *t = (const struct nk_command_colored_text*)cmd;
			nk_draw_list_add_colored_text(&ctx->draw_list, t->font, nk_rect(t->x, t->y, t->w, t->h),
				t->colorString, t->stringLen, t->colorString + t->stringLen + 1, t->colorLen, t->colorTable, t->height, t->background);
		} break;																		   
...
nk_font_text_width(nk_handle handle, float height, const char *text, int len)
{
    ...
    while (text_len <= (int)len && glyph_len) {
        ...
        /* query currently drawn glyph information */
		if (unicode == '\t') {
			g = nk_font_find_glyph(font, ' ');
			text_width += g->xadvance * scale * NK_TAB_SIZE;
		}
		else {
    ...
    struct nk_rect b, int down)
{
    ...
    if (!i) return nk_false;
    btn = &i->mouse.buttons[id];
	int ret = nk_input_has_mouse_click_in_rect(i, id, b) && (btn->down == down);
	return ret;
}
    ...
    struct nk_rect b)
{
    ...
    if (!i) return nk_false;
    btn = &i->mouse.buttons[id];
	int ret = (nk_input_has_mouse_click_down_in_rect(i, id, b, nk_false) &&
...
NK_INTERN void
nk_widget_colored_text(struct nk_command_buffer *o, struct nk_rect b,
	const char *string, int stringLen, const char* colors, int colorsLen, const struct nk_color* colorTable,
	const struct nk_text *t, nk_flags a, const struct nk_user_font *f)
{
	...
	text_width = f->width(f->userdata, f->height, (const char*)string, stringLen);
	text_width += (2.0f * t->padding.x);
	...
	nk_draw_colored_text(o, label, (const char*)string,
		stringLen, colors, colorsLen, colorTable, f, t->background);
}
...
/*
NK_INTERN int
nk_double_click_behavior(nk_flags *state, struct nk_rect r,
	const struct nk_input *i, enum nk_button_behavior behavior)
{
	int ret = 0;
	nk_widget_state_reset(state);
	...
	if (nk_input_is_mouse_hovering_rect(i, r)) {
		*state = NK_WIDGET_STATE_HOVERED;
		if (nk_input_is_mouse_down(i, NK_BUTTON_DOUBLE_CLICK))
			*state = NK_WIDGET_STATE_ACTIVE;
		if (nk_input_has_mouse_click_in_rect(i, NK_BUTTON_DOUBLE_CLICK, r)) {
			//Is this needed for a double click?
			ret = (behavior != NK_BUTTON_DEFAULT) ?
				nk_input_is_mouse_down(i, NK_BUTTON_DOUBLE_CLICK) :
#ifdef NK_BUTTON_TRIGGER_ON_RELEASE
				nk_input_is_mouse_released(i, NK_BUTTON_DOUBLE_CLICK);
#else
				nk_input_is_mouse_pressed(i, NK_BUTTON_DOUBLE_CLICK);
#endif
		}
	}
	if (*state & NK_WIDGET_STATE_HOVER && !nk_input_is_mouse_prev_hovering_rect(i, r))
