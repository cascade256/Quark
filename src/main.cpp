...
#error "Unsupported platform!"
	...
	//initLayout();
	ViewLayout* view = createSplitViewPixel(createView(drawFileTree, "File Tree"), createView(drawTextEditor, "Text Editor"), false, true, 300);
	int width;
	int height;
	glfwGetWindowSize(g->win, &width, &height);
	initLayout();
	layoutViews(view, width, height);
	arrayTest();
	...
	while (!glfwWindowShouldClose(g->win)) {
		...
		drawMainMenu();
		drawLayout(nk_window_get_content_region(g->ctx).w, nk_window_get_content_region(g->ctx).h);
		nk_end(g->ctx);
